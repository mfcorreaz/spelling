...
// ---- Si el torneo ya finalizó, no se permite seguir cronometrando/editando ----
if ($competencia['estado'] === 'finalizada') {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Torneo finalizado - Spelling Bee</title>
        <link rel="stylesheet" href="assets/css/panel_control.css">
        <link rel="stylesheet" href="assets/css/resultados_nivel.css">
    </head>
    <body class="panel-kiosko" style="justify-content:center; align-items:center; text-align:center;">
        <div>
            <h1 class="nombre-torneo">🏆 Este torneo ya finalizó</h1>
            <p style="color:#8B93A7; margin-bottom:20px;">No se pueden cargar ni modificar tiempos de un torneo finalizado.</p>
...
// Si por algún motivo no existe la fila de configuración, se crea sola
// (esto no debería pasar si el torneo se armó con el wizard, pero
// evita que el jurado quede trabado sin poder guardar nada).
...
if ($round_row) {
    ...
    $stmt = $pdo->prepare("INSERT INTO competencia_rounds (competencia_id, nivel_id, numero_round, cantidad_pasan) VALUES (?, ?, ?, NULL)");
    $stmt->execute([$competencia_id, $nivel_id, $round_actual]);
    $competencia_round_id = $pdo->lastInsertId();
}
...
if ($actual && $competencia_round_id) {
    $stmt = $pdo->prepare("
        SELECT id, tiempo_deletreo, tiempo_oracion, acierto_deletreo, penalizacion_segundos
...
<body class="panel-kiosko">
    ...
    <main class="panel-principal">
        ...
        <div class="fila-acciones">
            ...
            <?php endif; ?>
            <button type="button" id="btn-correcto" class="btn-accion btn-correcto" <?= $actual ? '' : 'disabled' ?>>✓ Correcto</button>
            <button type="button" id="btn-incorrecto" class="btn-accion btn-incorrecto" <?= $actual ? '' : 'disabled' ?>>✗ Incorrecto</button>
        </div>
    </main>
    ...
    <?php if ($actual && $competencia_round_id): ?>
    <p class="ayuda-teclado"><a href="#" id="btn-reiniciar-registro" class="link-reiniciar-registro">↺ Este alumno quedó en un estado raro / borrar y empezar de nuevo</a></p>
    <?php endif; ?>
    ...
    <input type="hidden" id="dato-inicio-absoluto" value="<?= ($nivel_id == ($levels[0]['id'] ?? null) && $round_actual == 1 && $idx == 0) ? '1' : '0' ?>">
