...
$palabra_id = (int)($datos['palabra_id'] ?? 0);
$tiempo_deletreo = (float)($datos['tiempo_deletreo'] ?? 0);

if (!$inscripcion_id || !$competencia_round_id || !$palabra_id || $tiempo_deletreo <= 0) {
...
if ($modo === 'parcial') {
    ...
    if ($existente) {
        $stmt = $pdo->prepare("UPDATE resultados SET tiempo_deletreo = ?, palabra_id = ? WHERE id = ?");
        $stmt->execute([$tiempo_deletreo, $palabra_id, $existente['id']]);
        $resultado_id = $existente['id'];
    } else {
        $stmt = $pdo->prepare("
            INSERT INTO resultados (inscripcion_id, competencia_round_id, palabra_id, tiempo_deletreo, registrado_por_usuario_id)
            VALUES (?, ?, ?, ?, ?)
        ");
        $stmt->execute([$inscripcion_id, $competencia_round_id, $palabra_id, $tiempo_deletreo, $_SESSION['usuario_id']]);
...
if ($existente) {
    $stmt = $pdo->prepare("
        UPDATE resultados SET
            palabra_id = ?, tiempo_deletreo = ?, acierto_deletreo = ?,
    ...
    $stmt = $pdo->prepare("
        INSERT INTO resultados
            (inscripcion_id, competencia_round_id, palabra_id, tiempo_deletreo, acierto_deletreo,
