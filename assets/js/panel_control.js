...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function guardarParcialDeletreo() {
    ...
    if (etapaActual !== 'deletreo') {
      mostrarMensaje('El tiempo de deletreo ya fue guardado. Seguí con la oración.', 'error');
      return false;
    }
    pausar();
    ...
    // Ocultamos el botón para que no se pueda volver a apretar (evita pisar el tiempo)
    const btn = document.getElementById('btn-guardar-deletreo');
  ...
  const inicioAbsoluto = document.getElementById('dato-inicio-absoluto').value === '1';
  ...
  function irAAnterior() {
    ...
    if (link && !link.classList.contains('nav-disabled')) {
      window.location.href = link.href; // hay un alumno anterior en el mismo level
    } else if (!inicioAbsoluto) {
      window.history.back(); // cruza a la pantalla de resultados del level/round anterior
    }
    // si es el inicio absoluto del torneo, no hace nada (se queda)
  }
  ...
  // ---- Botón de emergencia: borra el resultado de este alumno y recarga limpio ----
  const btnReiniciarRegistro = document.getElementById('btn-reiniciar-registro');
  if (btnReiniciarRegistro) {
    btnReiniciarRegistro.addEventListener('click', async (e) => {
      e.preventDefault();
      if (!confirm('¿Borrar el registro de este alumno en este round y empezar de nuevo?')) return;
      try {
        const resp = await peticion('reiniciar_registro.php', {
          method: 'POST',
          body: JSON.stringify({ inscripcion_id: inscripcionId, competencia_round_id: competenciaRoundId }),
        });
        if (resp.ok) {
          window.location.reload();
        } else {
          mostrarMensaje('No se pudo reiniciar: ' + (resp.error || ''), 'error');
        }
      } catch (err) {
